...
defineJourney(
  {
    ...
    catches: [
      ...
      "a shared-registry consumer selects a sibling resource type because review checked cardinality but not semantic filtering",
      ...
      while (Date.now() < journeyDeadline && terminalObservedAt.size < requirements.length) {
        ...
        for (const requirement of requirements) {
          ...
          if (run.status === "budgetExhausting" || run.status === "budgetExhausted") {
            throw new Error(`${requirement.id} exhausted its ten-minute active wall-clock budget`);
          }
...
function concurrentRequirements(
  ...
  const common = `这是同一项目内三个并行需求之一。项目初始化和共享拓扑已完成且 phase=active；不要再次执行 pm project init/advance/lifecycle，不要重新注册、重建或记录既有 Space。只管理当前 PM Session 的 Intent、Run、TeamSlot 和 WorkPackage；不得读取、推进、取消或复用其他 PM Session 的包。十分钟主动执行预算、累计 WorkSession 上限与并发 WorkSession 上限不可延长；V3 不把来源不明的推断请求数当硬门禁。用户已给出最终验收、包划分、分支、AgentSpace workspace id、Agent 与模型；这些信息是完整且可信的派工合同。按系统规则各读取必要 PM Skill 一次后直接派工；除此之外不要调用 agent list、help/schema、ls/find/grep/read，也不要读取产品源码、目录或其他 Session 状态。

PM 只做目标、团队和预算管理：不得读取或修改 repositories/、worktrees/ 中的代码，不运行项目测试，不执行技术评审，不生成 verdict，也不执行 Git 集成。实现与自测由 implementation WorkAgent 完成；候选由另一个 review Space 中的独立 Reviewer 复验。Candidate 出现后，项目固定的真实 \`activity: review / actor: reviewer\` 节点会生效；Supervisor 只在节点已解析出可用 \`reviewWorkspace\` 时唤醒 PM，PM 不得对局部 cohort 猜测或修复评审容量。收到 \`动作=派发独立评审\` 后，不再查询 status/show；把本批全部原 WorkPackage id 放在同一次 genehub 批量调用中，启动消息统一使用“按 Run 固定合同独立评审精确候选”。Coordinator 会把项目版本化的中文 review 提示词、精确 Intent、包边界、实现合同和 candidate commit/tree 注入 Reviewer 系统合同，PM 不得复制、扩写或重写技术评审合同。不得新建 Review WorkPackage、绕过 review 节点或等待 Coordinator 代替团队派发；Reviewer selector、节点提示词和 capacity 都来自本 Run 固定的项目 Workflow，Coordinator 只校验结果、推导事实和推进图。切片 Reviewer 只能因绑定候选自身造成的验收缺陷或明确的集成 seam 判失败：必须拒绝候选中依赖兄弟切片暂缺才成立的偶然绿色断言；切片局部数量、唯一性和枚举范围必须针对切片自己拥有的导出或 registry 断言，共享聚合入口会随兄弟包合入而变化，若候选把共享聚合入口的精确数量断言为本切片数量，必须 review-fail。不得仅因责任兄弟包尚未合入、导致最终组合门禁暂时不可观测，就把当前切片判失败或要求它修改责任外路径；本包边界用 fixture/动态探针复验，完整 npm test/build 由合流后系统门禁负责。Coordinator 注入的 Reviewer 合同以约 6-12 次 LLM 请求为性能目标而非来源不明的硬门禁，最多三组只读合并命令：第一组一次取得 HEAD/tree/status/diff/行数等身份与范围证据，第二组一次执行全部定向测试和 build，必要时第三组只补一个失败点；不得逐文件通读确定性生成的同构模块。Reviewer 必须保持候选 worktree 只读：不得 checkout/revert/reset，不得复制候选后通过回退文件反演 baseline，不得写入或清理候选；若命令修改 tracked 文件或留下非忽略文件，必须 review-fail。该约束只提高取证效率，不允许 Reviewer 省略独立命令或让 PM 代评。Worker/Reviewer 末尾的结构化结果由 Coordinator 读取，Coordinator 自行推导 commit/tree、推进 Candidate/Accepted 并在 \`activity: integrate\` 节点串行合入 main。每份 Worker/Reviewer 合同都必须保留系统结果协议：最后一个非空行是唯一的 \`GENEHUB_WORK_RESULT {...}\`，其后不能有文字；实现只允许 status=\`candidate-ready\` 或 \`blocked\`；review-pass 必须使用最小对象 \`{"status":"review-pass","summary":"精确候选的全部门禁通过"}\` 且不带 findings，review-fail 必须使用 \`{"status":"review-fail","summary":"仍有验收缺陷","findings":[{"severity":"blocking|high|medium|low","title":"...","acceptanceImpact":"...","recommendedAction":"...","estimatedRequests":1}]}\`，findings 只能是对象数组，recommendedAction 必须位于每个 finding 内。\`candidate\` 和 \`failed\` 不是协议值。禁止用“首行 review-pass”或自定义 RESULT 取代它。Reviewer 标记缺失或格式错误时由 Coordinator 自动且仅修复一次；PM 不得续发协议修复、不得自己合成 verdict。PM 在 review-fail 时只依据验收影响、Reviewer 的预计请求数和本 Run 剩余执行预算选择有界返工或升级用户，不得覆盖 Reviewer verdict。

每个 package put 必须使用消息中给出的完整 argv（包括 --id/--title/--outcome/--repository/--branch/--node/--space-tag）；不得省略字段或重复 implementation 等节点基础标签。Coordinator 返回的 worktree 已由用户创建并在实现/评审 Workspace 中精确注册；不要再建 worktree。只用 OpenCode + ${workModel}，PM 保持 ali/qwen3.8-flash medium。把消息列出的全部命令按顺序放在同一次内置 genehub 批量工具调用中；不要改用 bash。一个 fanout 必须先执行全部 package put，再执行全部 Ready，最后执行全部 agent run；禁止 put A → Ready A → run A → put B，因为第一个 Ready 会封闭 cohort。\`agent run\` 已带 --no-wait，成功返回时 Coordinator 会原子绑定 WorkSession，禁止再手工绑定或伪造状态。派发完立即结束回合，等待 Supervisor 批量唤醒；不要轮询运行中 Session。候选出现后只按 Supervisor 的精确目标批量启动独立 Reviewer，不查询状态、不编写评审细节；review-pass 后停止操作并等待 Workflow 进入用户决策节点，用户批准精确候选后才由 Coordinator 接受并集成。`;
...
必须用同一次 genehub 批量调用执行：workflow select bugfix；intent set（验收含三个缺陷各自定向测试、独立 Reviewer、main 上 npm test、至少两个实现 WorkSession 同时 Running，首次 Intent 不传 --affects）；workflow transition aligned + intent.aligned；然后在 fix 节点严格按三阶段派发三个包：
