...
for (const scenario of ["plain", "multiple-repos", "own-worktree", "parent-repo", "formal-metadata", "formal-alternates", "silence-wr", "readonly-unsupported"] as const) defineSpecialty({
  ...
  expectedDurationMs: scenario === "silence-wr" ? 210000 : 15000, timeoutMs: scenario === "silence-wr" ? 300000 : 90000,
  ...
  try {
    ...
    const diagnosis = scenario === "silence-wr";
    const dataOnly = scenario === "plain" || diagnosis || scenario === "readonly-unsupported";
    const good = ["plain", "multiple-repos", "own-worktree", "silence-wr"].includes(scenario);
