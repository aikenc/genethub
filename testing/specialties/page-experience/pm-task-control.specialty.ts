...
defineSpecialty({
  ...
  tags: ["page-experience", "workflow-control", "session-attention", "session-control-fixes"], runner: "playwright", llm: { default: "mock" },
  ...
  try {
    ...
    await task.getByRole("button", { name: /小队任务/ }).waitFor();
    await page.getByRole("button", { name: "发送", exact: true }).waitFor();
    t.assertions.assert(await task.getByLabel("PM 本轮状态").count() === 0, "squad panel still owns Agent conversation state");
    await page.getByRole("textbox", { name: "任务描述" }).fill("保留 PM 草稿");
    t.assertions.assert(await task.boundingBox().then(box => !!box && box.height < 70), "collapsed task panel consumes the conversation");
    await task.getByRole("button", { name: /小队任务/ }).click();
    const taskDialog = page.getByRole("dialog", { name: "小队任务", exact: true });
    await taskDialog.getByRole("button", { name: /查看 .* 待处理问题/ }).click();
    await page.getByRole("button", { name: "返回", exact: true }).click();
    await task.waitFor();
    t.assertions.assert(await page.getByRole("textbox", { name: "任务描述" }).inputValue() === "保留 PM 草稿", "nested session back lost PM draft");
    await task.getByRole("button", { name: /小队任务/ }).click();
    await taskDialog.getByText(/由上级 Agent 处理。/).waitFor();
    ...
    await taskDialog.getByRole("button", { name: /查看 .* 待处理问题/ }).waitFor();
    await taskDialog.getByRole("button", { name: "关闭小队任务" }).click();
    const input = page.getByRole("textbox", { name: "任务描述" });
    const send = async (text: string) => { await input.fill(text); await page.getByRole("button", { name: /^(发送|发送补充消息)$/ }).click(); };
    ...
    await task.getByRole("button", { name: /小队任务/ }).click();
    await taskDialog.getByRole("button", { name: "终止任务", exact: true }).click();
    await t.tools.waitUntil(async () => (await snapshot()).summary.workSummary?.tasks[0]?.status === "cancelled", 35_000);
    await taskDialog.getByText(/ · 已取消$/, { exact: false }).waitFor();
    await taskDialog.getByRole("button", { name: "关闭小队任务" }).click();
    await task.getByRole("button", { name: /已取消/ }).waitFor();
    ...
    t.assertions.assert(pmCalls === beforeCancelCalls, "direct cancellation woke PM for a new LLM turn");
    t.assertions.assert(!(await snapshot()).summary.workSummary?.tasks[0]?.reportPending, "cancelled task still awaits a PM report");
