...
Editable definitions currently live in `<project>/.genethub/workflow/`. Their
`project.yaml.execution` selects the Executor and task directory. That directory
is a weak task reference; PM → Executor → direct Workers is an ownership tree.
