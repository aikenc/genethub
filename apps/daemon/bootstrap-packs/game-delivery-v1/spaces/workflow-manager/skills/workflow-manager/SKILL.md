...
Before changing `agent.session` roles, inspect the Candidate's selected Executor and its direct Workers with `space children`. Historical Executors describe historical Runs and are not the new Candidate's role inventory. A role file does not create a Worker. Exactly one enabled direct Worker per referenced role is needed before dispatch, but a preparation plan may request new roles that PM has yet to create. Reuse appropriate existing roles; name and describe any new Worker, Skill, model and permission requirement explicitly.
