...
For `game-review-and-improve`, read [the review contract](references/review-contract.md). Review the existing artifact first; freeze item IDs and acceptance, cover every criterion, validate the structured report, and preserve that acceptance across repair and re-review. Initial approval needs no Coder commit. Report references and actual checks are required alongside the approval verdict. The Workflow allows at most two repair/re-review rounds in the same Run.
