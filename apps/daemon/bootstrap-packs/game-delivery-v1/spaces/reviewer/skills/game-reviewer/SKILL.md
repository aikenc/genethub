...
Submit a complete negative result when acceptance fails: `"$GENEHUB_CLI" workflow complete --outcome changesRequested --reason <precise-finding> --evidence checks=<actual-report>`. This completes the review work while returning the business defect to PM; do not leave the node running or invent `review=approved`. Successful acceptance continues to use the Workflow's required evidence contract.
