...
pub fn valid_space_name(value: &str) -> bool {
    manifest::valid_name(value)
}
...
impl From<Diagnostic> for genehub_proto::AgentSpaceBuilderDiagnostic {
    fn from(diagnostic: Diagnostic) -> Self {
        Self {
            ...
            target: diagnostic.target.map(Into::into),
            semantic_key: diagnostic.semantic_key.map(Into::into),
            action: diagnostic.action.map(Into::into),
        }
    }
}
...
impl From<Report> for genehub_proto::AgentSpaceBuilderReport {
    fn from(report: Report) -> Self {
        Self {
            ...
            pipespace_root: report.pipespace_root,
...
fn init(project_root: &Path, requested_root: &Path) -> BuilderResult<Report> {
        ...
        if is_symlink(&spaces_path) {
            return fail(
                Diagnostic::error("PB011", "project spaces/ must not be a symlink")
                    .source(spaces_path.display().to_string()),
            );
        }
        std::fs::create_dir_all(&spaces_path).map_err(io_error)?;
    }
    let spaces = spaces_path.canonicalize().unwrap_or(spaces_path);
    ...
    if root != project_root && root.parent() != Some(spaces.as_path()) {
        return fail(
...
fn validate_space_root(project_root: &Path, space_root: &Path) -> BuilderResult<PathBuf> {
    ...
    if !root.is_dir()
        || (root != project_root && root.parent() != Some(spaces.as_path()))
...
mod tests {
    ...
    #[test]
    fn project_root_and_direct_child_share_one_builder_contract() {
        ...
        run(&project, &project, Command::Init, true).expect("initialize project AgentSpace");
        run(&project, &project, Command::Build { dry_run: false }, true)
            .expect("build project AgentSpace");
        let root = verify_space(&project, &project).expect("verify project AgentSpace");
        assert_eq!(root.name, "game-project");

        let coder = project.join("spaces/coder");
        run(&project, &coder, Command::Init, true).expect("initialize child AgentSpace");
        run(&project, &coder, Command::Build { dry_run: false }, true)
            .expect("build child AgentSpace");
        let child = verify_space(&project, &coder).expect("verify child AgentSpace");
        assert_eq!(child.name, "coder");
        assert_eq!(child.workspace_path, coder.join("coder.code-workspace"));
    }

    #[test]
    fn generated_artifact_drift_is_not_a_verified_space() {
        let temp = tempfile::tempdir().expect("temporary project");
        let project = temp.path().join("game-project");
        std::fs::create_dir(&project).expect("project root");
        let project = project.canonicalize().expect("canonical project");
        ...
        run(&project, &coder, Command::Build { dry_run: false }, true)
            .expect("build child AgentSpace");
        std::fs::write(coder.join("AGENTS.md"), "tampered\n").expect("tamper generated file");

        let error = verify_space(&project, &coder).expect_err("drift must be refused");
