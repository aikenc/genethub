...
pub struct Diagnostic {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub semantic_key: Option<Box<str>>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Diagnostic {
    pub fn error(code: &'static str, message: impl Into<String>) -> Self {
        Self {
            ...
            target: None,
            semantic_key: None,
            action: action_for(code).map(Box::<str>::from),
        }
    }
    ...
    pub fn target(mut self, target: impl Into<String>) -> Self {
        self.target = Some(target.into().into_boxed_str());
        self
    }

    pub fn semantic_key(mut self, key: impl Into<String>) -> Self {
        self.semantic_key = Some(key.into().into_boxed_str());
        self
    }

    pub fn action(mut self, action: impl Into<String>) -> Self {
        self.action = Some(action.into().into_boxed_str());
        self
    }
}
