...
fn process_alive(pid: u32) -> bool {
    // Native Unix can prove a stale pid. The WASI guest deliberately has no
    // arbitrary-process probe in its host contract, so the shared lifecycle
    // helper answers conservatively there and the lock stays fail-closed.
