...
mod tests {
    ...
    async fn tclaude_asks_upstream_help_through_the_wrapper_separator() {
        ...
        // This process emits no help, so the merged adapter must also preserve
        // the wrapper separator on every fallback probe. Keep all invocations
        // rather than overwriting the evidence with the last probe.
        assert_eq!(
            args,
            concat!(
                "--\n--help\n",
