...
mod tests {
    ...
    #[test]
    fn builder_flags_are_fail_closed() {
        ...
        .unwrap();
        assert_eq!(args.space, "gameplay");
        assert_eq!(args.command, Command::Build { dry_run: true });
        assert!(args.require_no_post_commands);
        assert!(Args::parse(&strings(&["verify", "gameplay", "--dry-run"])).is_err());
        assert!(Args::parse(&strings(&["build", "gameplay", "--wat"])).is_err());
        assert!(Args::parse(&strings(&["build", "gameplay", "--dry-run", "--dry-run"])).is_err());
        let init = Args::parse(&strings(&["init", "gameplay"])).unwrap();
        assert_eq!(init.command, Command::Init);
        assert!(init.mutates());
    }

    #[test]
    fn space_name_cannot_select_another_path() {
        let root = std::path::Path::new("/project");
        assert_eq!(
            space_root(root, "gameplay").unwrap(),
            root.join("spaces/gameplay")
        );
        assert!(space_root(root, "../other").is_err());
        assert!(space_root(root, "spaces/gameplay").is_err());
        assert!(space_root(root, "/tmp/gameplay").is_err());
    }
}
