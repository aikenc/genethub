...
// A product Space directory is `<flat-package-id>--<space>`, so the separator
// is part of a legitimate name. The pattern still refuses path navigation,
// uppercase and anything that is not a plain lowercase segment.
...
// The trial's carriers are the product directories its own package derives,
// so they are named for that package rather than suffixed by hand.
const executorName = `${plan.package}-${plan.name}--${sourceManifest.name}`.replaceAll("/", "-");
...
const members = sourceMembers.map((member, index) => {
  ...
  const name = `${plan.package}-${plan.name}--${manifest.name}`.replaceAll("/", "-");
  ...
  // The manifest keeps the Space's local name — the package prefix lives in
  // the product directory, exactly as `workflow build` materializes it.
  ...
  workspace.folders = [{ name: localName, path: "." }, { name: "test-material", path: path.relative(target, taskRoot).split(path.sep).join("/") }];
  files.push({ target: path.join(target, `${localName}.code-workspace`), bytes: Buffer.from(encode(workspace)) });
  return { name, localName, root: target, sourceId: member.id, lifecycle: member.agentSpace.lifecycle, components: member.agentSpace.components, executor: index === 0 };
});
...
// There is no project-level execution binding to rewrite any more: a trial's
// carrier is the one its own package derives, and the task directory travels
// with the Run as `workflow dispatch --root`. The receipt reports both so PM
// dispatches the trial explicitly instead of mutating shared configuration.
const binding = {
  executorPath: `spaces/${executorName}`,
  root: path.relative(projectRoot, taskRoot).split(path.sep).join("/"),
};
const receipt = previous ?? { schema: "genehub.workflow-preparation.v1", inputDigest, sourceDigest, binding, repositories, members: [], builds: {}, changes: {} };
...
for (const member of members) {
  ...
  const opened = genet(["space", "open", path.join(member.root, `${member.localName}.code-workspace`)]).workspace;
...
// A variant is its own package: copying the source directory is what gives it
// an independent definition whose derived carrier is the copied team, rather
// than the formal one. Without it the trial Candidate would still name the
// formal Executor and the kernel would refuse it as unisolated.
const sourcePackage = path.join(projectRoot, ".genethub", "workflows", plan.package ?? "");
if (!plan.package || !existsSync(sourcePackage)) {
  throw new Error("Name the Workflow package this trial varies with plan.package");
}
const trialPackageId = `${plan.package}-${plan.name}`;
const trialPackage = path.join(projectRoot, ".genethub", "workflows", trialPackageId);
noLinks(projectRoot, trialPackage);
if (!existsSync(trialPackage)) {
  const copyTree = (from, to) => {
    ...
    for (const entry of readdirSync(from, { withFileTypes: true })) {
      ...
      else throw new Error(`Copy only regular package files: ${source}`);
    }
  };
  copyTree(sourcePackage, trialPackage);
}
const candidate = genet(["workflow", "inspect", "--workspace", project.id, "--package", trialPackageId]);
if (!candidate.candidateDigest) throw new Error(candidate.candidateError ?? "Candidate did not compile");
// The variant is a package with its own activation pointer, so preparing it
// means activating that pointer. The formal package's pointer is untouched:
// a trial can no longer promote itself into the mainline by construction.
if (candidate.activeDigest !== candidate.candidateDigest) {
  genet(["workflow", "activate", "--workspace", project.id, "--package", trialPackageId,
    "--revision", String(candidate.activationRevision ?? 0)]);
}
receipt.prepared = true;
receipt.packageId = trialPackageId;
receipt.candidateDigest = candidate.candidateDigest;
receipt.executorWorkspaceId = executorId;
receipt.executionRoot = taskRoot;
save();
process.stdout.write(encode({ ...receipt, receiptFile }));
