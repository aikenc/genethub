...
若问题发生在已加载客户端联调运行时的 GeneHub 页面，可使用内置 `genehub-client-debug` 检查页面状态、
错误、DOM 或重建截图；通过联调 inspect 确认预览 iframe 的同源边界。不向用户静态 HTML 注入联调脚本，
不把 DOM 截图当成跨源媒体或 GPU 画面的原生截图。文件路径/加载器问题仍按本 Skill 检查。
