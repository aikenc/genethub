...
description: 为内容工作者搭建、登记、诊断和分享创作过程预览。用户要求搭建、对接或落地可预览服务时，先复制本 Skill 的 python-adapter、用 GENEHUB_CLI daemon status 取 dataDir、启动登记并让用户从后台运行打开预览；不要只写架构说明。用于影视剪辑与合成、DCC 建模/材质/动画/仿真、游戏引擎运行与交互、数字人制作和实时驱动，以及这些流程需要的本地 HTTP/WS 服务和原生 WebRTC。纯静态 H5、相册和文件预览使用 genehub-html-preview。
---
...
用户说「搭建预览」「对接预览」「可预览」时，**先落地一条能打开的登记服务**，再替换成真实管线。不要先写长文再等用户催落地。
...
4. **启动登记。** 在工作区虚拟环境安装 `preview-adapter/requirements.txt` 后运行（路径换成实际值）：
   ...
   Windows 用 `.venv\Scripts\python.exe` 和该机的入口绝对路径。先让示例跑通「后台运行 → 打开预览 → 允许访问登记服务」。数字人/UE 的 offer 桥、口型、PCM 适配，都放在这条基线通了之后，见[数字人接入](references/digital-human.md) / [UE 接入](references/unreal-engine.md)。
5. **把体验入口交给用户。** 链入口 HTML，不链目录。明确三下：打开 **后台运行** → 选中该工作区并刷新 → 点服务节点上的 **打开预览**；可信工具栏再点 **允许本次预览访问登记服务**。`files` 不能代替 `services`。可用 `"<GENEHUB_CLI>" process.list` 确认适配器进程还在。
6. **完成标准。** 后台运行里能看到该工作区的服务节点之前，不要说「已经可预览」。健康接口或 SDP answer 只证明对应的一层。数字人/引擎还要用户在可信媒体面板看到真实画面（或明确报告「媒体基线已通，内容管线未接」）。
...
给出入口链接、源机器/Channel、`dataDir` 来源（`daemon status`）、启动/停止方式、已验证行为及当前运行期限。区分阶段产物与实时画面、参考图案与实际内容、观看与操作、本机与跨网结果。
