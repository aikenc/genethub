...
description: 为内容工作者搭建、登记、诊断和分享创作过程预览。用于影视剪辑与合成、DCC 建模/材质/动画/仿真、游戏引擎运行与交互、数字人制作和实时驱动，以及这些流程需要的本地 HTTP/WS 服务和原生 WebRTC。引导选择阶段产物、进度面板、实时音视频或操作回传，接入语言无关登记、后台进程树、服务授权、媒体契约和 ICE/TURN，并验证体验入口。纯静态 H5、相册和文件预览使用 genehub-html-preview。
---
...
- 自行编写接入程序：读[语言无关登记与协议](references/registration-contract.md)，不要求固定 Node runner。
...
2. 检查已有输出、插件/API、运行环境和硬件。选择满足需求的最小接入；软件名称或“支持 WebRTC”不构成兼容证明。需要后端时确认实际 daemon 数据目录及接入协议，它们与工作区、Skill 目录不同。
...
- 应用登记对应一次运行；可选 runner 托管自己启动的前台后端，不是常驻服务管理器或 OS 沙箱。不要假定能接管用户已经打开的 DCC/编辑器；对已有应用的连接、停止及未保存内容分别处理。
- 已授权的搭建任务可以继续必要的本地工作。公网托管、付费资源、上传用户媒体或修改驱动/系统安全设置需要相应授权；不要对已经授权的工作重复要求确认。
- 不手改私有登记来冒充可用应用。先正常停止；确认旧运行及其进程已退出后才能清理陈旧登记。私有记录、TURN 凭证和用户音视频不进入共享产物。
