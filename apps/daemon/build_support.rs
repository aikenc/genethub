...
/// Scans versioned Bootstrap Packs without assigning business meaning to
/// their contents. Each top-level directory is one pack and must own a
/// regular `pack.json`; runtime schema validation remains in the daemon.
pub(crate) fn scan_bootstrap_tree(root: &Path) -> Result<ResourceTree, String> {
    if !root.is_dir() {
        return Err(format!("{} is not a directory", root.display()));
    }
    let mut directories = vec![root.to_path_buf()];
    let mut files = Vec::new();
    let mut packs = read_entries(root)?;
    packs.sort();
    if packs.is_empty() {
        return Err("the Bootstrap Pack tree is empty".into());
    }
    for pack in packs {
        let metadata = std::fs::symlink_metadata(&pack)
            .map_err(|error| format!("reading {}: {error}", pack.display()))?;
        if metadata.file_type().is_symlink() || !metadata.is_dir() {
            return Err(format!(
                "{} must be a real top-level Bootstrap Pack directory",
                pack.display()
            ));
        }
        let pack_json = pack.join("pack.json");
        let metadata = std::fs::symlink_metadata(&pack_json)
            .map_err(|_| format!("{} has no regular pack.json", pack.display()))?;
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(format!("{} must be a regular file", pack_json.display()));
        }
        collect_files(root, &pack, &mut directories, &mut files)?;
    }
    directories.sort();
    directories.dedup();
    files.sort();
    files.dedup();
    Ok(ResourceTree { directories, files })
}
...
pub(crate) fn render_bootstrap_manifest(tree: &ResourceTree) -> String {
    let mut generated = String::from(
        "// @generated by apps/daemon/build.rs. Do not edit.\n\
    ...
    for relative in &tree.files {
        writeln!(
            generated,
            "    BootstrapFile {{ relative_path: {relative:?}, contents: include_bytes!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/bootstrap-packs/\", {relative:?})) }},"
        )
        .expect("writing to a String cannot fail");
    }
    generated.push_str("];\n");
...
mod tests {
    ...
    fn write_pack(root: &Path, directory: &str) -> PathBuf {
        let pack = root.join(directory);
        std::fs::create_dir_all(pack.join("assets")).unwrap();
        std::fs::write(
            pack.join("pack.json"),
            format!(r#"{{"schema":"genehub.bootstrap-pack.v1","id":"{directory}"}}"#),
        )
        .unwrap();
        std::fs::write(pack.join("assets/resource.txt"), "resource").unwrap();
        pack
    }
    ...
    #[test]
    fn bootstrap_scan_discovers_and_embeds_pack_resources() {
        let temp = TempTree::new("bootstrap-valid");
        let root = temp.0.join("bootstrap-packs");
        write_pack(&root, "zeta");
        write_pack(&root, "alpha");

        let tree = scan_bootstrap_tree(&root).unwrap();
        assert!(tree
            .directories
            .iter()
            .any(|path| path.ends_with("alpha/assets")));
        assert_eq!(
            tree.files,
            [
                "alpha/assets/resource.txt",
                "alpha/pack.json",
                "zeta/assets/resource.txt",
                "zeta/pack.json",
            ]
        );
        let generated = render_bootstrap_manifest(&tree);
        assert!(generated.contains("zeta/assets/resource.txt"));
        assert!(generated.contains("BOOTSTRAP_FILES"));
        assert!(generated.contains("include_bytes!"));
    }

    #[test]
    fn bootstrap_scan_rejects_a_pack_without_manifest() {
        let temp = TempTree::new("bootstrap-missing-manifest");
        let root = temp.0.join("bootstrap-packs");
        std::fs::create_dir_all(root.join("broken")).unwrap();

        let error = scan_bootstrap_tree(&root).err().expect("invalid pack tree");
        assert!(error.contains("has no regular pack.json"));
    }
}
