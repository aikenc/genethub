...
//!
//! Each existing volume is preopened as `/c`, `/d`, … and host paths in the
//! environment are rewritten to that shape. Host imports that take a path
//! from the guest translate back before touching the real filesystem.
...
mod tests {
    ...
    fn guest_posix_volume_paths_round_trip_to_windows() {
        assert_eq!(
            host_path_from_guest("/c/Users/RUNNER~1/AppData/Local/Temp/.tmp8MctaG"),
            PathBuf::from(r"C:\Users\RUNNER~1\AppData\Local\Temp\.tmp8MctaG")
        );
        assert_eq!(
            host_path_from_guest("/d/a/genethub/genethub"),
    ...
    fn already_windows_paths_pass_through_on_the_host_side() {
        assert_eq!(
            host_path_from_guest(r"C:\Users\someone\GeneHub"),
