...
export interface FabricEndpointContext extends FabricEndpointGrant {
  ...
  /** Capability asserted on this physical WebSocket endpoint. */
...
export interface FabricPeerBinding {
  ...
  /** Both physical endpoints opted into independent TCP-leg backpressure. */
  transportFlow: boolean;
}
...
export class FabricCore {
  ...
  /** True only after route authorization proved both socket capabilities. */
  usesTransportFlow(
    connection: FabricEndpointConnection,
    streamId: string,
  ): boolean {
    return connection.streams.get(streamId)?.binding.transportFlow === true;
  }
  ...
  async handle(connection: FabricEndpointConnection, frame: FabricFrame): Promise<void> {
    ...
    switch (frame.kind) {
      ...
      case FabricKind.Data:
        await this.data(connection, frame);
        return;
  ...
  private async open(
    ...
    const transportFlow =
      source.context.transportFlow && target.context.transportFlow;
    ...
    target.send({
      ...
      value: transportFlow ? 0n : frame.value,
  ...
  private accept(connection: FabricEndpointConnection, frame: FabricFrame): void {
    ...
    const validCredit = binding.transportFlow
      ? frame.value === 0n
    ...
    peer.sendCredit = binding.transportFlow ? 0n : frame.value;
    peer.sendWindow = binding.transportFlow ? 0n : frame.value;
    peer.connection.send({
      kind: FabricKind.Accept,
      streamId: peer.localStreamId,
      value: binding.transportFlow ? 0n : frame.value,
      payload: frame.payload,
    });
  }
