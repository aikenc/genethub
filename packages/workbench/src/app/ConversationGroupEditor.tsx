...
import type { ConversationGroup } from "../session/conversationGroups";
import { WorkspaceDetailsDialog } from "../workspace/WorkspaceDetailsDialog";

export function ConversationGroupEditor({ group, groups, workspaces, error, onSave, onDelete, onClose }: {
  group?: ConversationGroup;
  groups: ConversationGroup[];
  workspaces: WorkspaceInfo[];
  error: string;
  onSave(group: ConversationGroup): boolean;
  ...
  return <WorkspaceDetailsDialog title={group ? "编辑分组" : "新建分组"} onClose={onClose}>
    ...
    <p className="mb-2 mt-5 font-medium">自动收集这些 Agent 的会话</p>
    <p className="mb-3 text-xs leading-5 text-muted">可多选，也可以不选，只手动加入会话。这里选择子 Agent 会明确包含它的会话。</p>
    ...
    <div className="max-h-48 overflow-y-auto rounded-lg border border-line">
      ...
      {!workspaces.length && <p className="p-3 text-muted">暂无 Agent，可先创建手动分组。</p>}
    </div>
    {agents.some((id) => !workspaces.some((w) => w.id === id)) && <p className="mt-2 text-xs text-muted">保留了暂不可用 Agent 的收集规则。</p>}
    ...
    <div className="mt-5 flex items-center justify-end gap-2">
      ...
      <button disabled={!valid} className="min-h-11 rounded-lg bg-accent px-4 text-white disabled:opacity-40" onClick={() => {
        if (onSave({ id: group?.id ?? crypto.randomUUID(), name: name.trim(), workspaceIds: agents, included: group?.included ?? [], excluded: group?.excluded ?? [] })) onClose();
      }}>保存分组</button>
    </div>
    {confirmDelete && <div className="mt-3 rounded-lg border border-line p-3">
      <p>删除「{group?.name}」？其中的会话和历史都会保留。</p>
