...
export function App({
  ...
  useEffect(() => { if (section === "spaces") setSpacesVisited(true); }, [section]);
    ...
    setLandingIntent(
      loc
        ? {
            ...
            localDraftId: window.history.state?.genehubPage?.draftLocalId,
            localDraftWorkspaceId: window.history.state?.genehubPage?.workspaceId,
  ...
  useEffect(() => {
    if (host.kind !== "browser") return;
    const apply = () => {
      if (window.history.state?.genehubPage) return;
      const loc = readWorkbenchLocation();
      if (!loc) return;
  ...
  useEffect(
    () =>
      host.onUpdateRequested?.(() => {
        ...
        // version section.
        void useWorkbench.getState().checkUpdates();
        useWorkbench.getState().openTab("settings");
      }),
    [host],
  );
                  ...
                  <>
                    <div className="min-h-0 flex-1 overflow-hidden">
                      {/* A draft has no transcript to show, and the room it
                          ...
                          hidden. */}
                      {starting ? (
                        <NewSessionPanel host={host} endpoint={endpoint} surface={overviewSurface} onSurface={setOverviewSurface} onBack={backPage} nested={nested} />
                      ) : (
                        ...
                        <div className="relative h-full">
                          <TimelineView
                            key={`${deviceHandle ?? endpoint.label}:${session?.id ?? "empty"}`}
                            readingKey={`${deviceHandle ?? endpoint.label}:${session?.id ?? "empty"}`}
                            visible={section === "sessions" && (!sessionsOpen || window.innerWidth >= 768)}
                    ...
                    {session && durableInput ? <InputDelivery session={session} /> : null}
