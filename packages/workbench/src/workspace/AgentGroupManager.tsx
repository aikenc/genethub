...
export function AgentGroupManager({ groups, workspaces, error, update, onClose }: {
  ...
  return <WorkspaceDetailsDialog title="项目分组" onClose={onClose}>
    <div className="flex items-center justify-between gap-3"><p className="text-sm text-muted">管理常驻项目，会话自动跟随分组。</p><button className={listPrimaryAction} onClick={() => setEditing("new")}>新建分组</button></div>
    <ul className="mt-3 divide-y divide-line">
      {groups.map((group) => <li key={group.id} className="flex min-w-0 items-center gap-3 py-2"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{group.name}</p><p className="text-xs text-muted">{group.workspaceIds.length} 个项目</p></div><button aria-label={`编辑 ${group.name}`} className="min-h-11 rounded-lg px-3 text-sm text-accent hover:bg-raised" onClick={() => setEditing(group.id)}>编辑</button></li>)}
    </ul>
    {!groups.length && <p className="py-8 text-center text-sm text-muted">还没有项目分组，先创建一个并勾选成员。</p>}
