...
export function AgentGroupEditor({ group, groups, workspaces, error, onSave, onDelete, onClose }: {
  ...
  return <WorkspaceDetailsDialog title={group ? "编辑分组" : "新建分组"} onClose={onClose}>
    ...
    <p className="mb-2 mt-5 font-medium">选择组内项目</p>
    <p className="mb-3 text-xs leading-5 text-muted">可多选，一个项目可以属于多个分组。会话按所属项目自动进入分组，子项目需单独选择。</p>
    <ListSearch label="搜索分组项目" value={query} onChange={setQuery}/>
    <div className="mt-2 max-h-64 overflow-y-auto rounded-lg border border-line">
      ...
      {!workspaces.length && <p className="p-3 text-muted">暂无项目，可先创建空分组。</p>}
    </div>
    {agents.some((id) => !workspaces.some((w) => w.id === id)) && <p className="mt-2 text-xs text-muted">保留了暂不可用项目的分组关系。</p>}
    ...
    {confirmDelete && <div className="mt-3 rounded-lg border border-line p-3">
      <p>删除「{group?.name}」？项目、会话和历史都会保留。</p>
