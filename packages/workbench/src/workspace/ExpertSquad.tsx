...
export function ExpertSquad({ workspace, onOpen }: { workspace: WorkspaceInfo; onOpen(id: string): void }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex gap-2"><select aria-label="添加已有专家到小队" value={selected} onChange={e => setSelected(e.target.value)} className="min-h-11 min-w-0 flex-1 rounded-lg border border-line bg-raised px-2 text-sm"><option value="">选择已有专家</option>{candidates.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select><button type="button" disabled={busy || !candidates.some(w => w.id === selected)} className="min-h-11 rounded-lg bg-accent px-3 text-sm text-white disabled:opacity-40" onClick={() => { const child = candidates.find(w => w.id === selected); if(child) void move(child, workspace.id); }}>加入</button></div>
