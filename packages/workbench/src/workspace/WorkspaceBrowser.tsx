...
import { localValue, draftIdentities, readLocalDraft } from "../session/localConversation";
...
export function WorkspaceBrowser({ host, endpoint, navigationKey, deviceName, initialWorkspaceId, extraTabs, onSession, onNewSession, onExtra, onDepthChange }: {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(initialWorkspaceId ?? window.history.state?.genehubSpace?.id ?? null);
  const [surface, setSurfaceState] = useState<string>(window.history.state?.genehubSpace?.surface ?? "sessions");
  const setSurface = (value: string) => {
    window.history.replaceState({...window.history.state, genehubSpace: {id: selectedId, surface}}, "");
    window.history.pushState({...window.history.state, genehubSpace: {id: selectedId, surface: value}, genehubParent: true}, "");
    setSurfaceState(value);
  };
  useEffect(() => { onDepthChange?.(Boolean(selectedId)); }, [selectedId, onDepthChange]);
  useEffect(() => {
    const restore = () => { const page = window.history.state?.genehubSpace; if (page) { setSelectedId(page.id); setSurfaceState(page.surface); } };
    window.addEventListener("popstate", restore); return () => window.removeEventListener("popstate", restore);
  }, []);
  const [query, setQuery] = useState("");
  useEffect(() => { setSelectedId(initialWorkspaceId ?? window.history.state?.genehubSpace?.id ?? null); setSurfaceState(initialWorkspaceId ? "sessions" : window.history.state?.genehubSpace?.surface ?? "sessions"); setQuery(""); }, [initialWorkspaceId, navigationKey]);
  ...
  const owned = sessions.filter(item => item.workspaceId === workspace?.id && !item.archived && !item.unsupported);
  const machine = client?.identity?.machineId ?? deviceName;
  const last = workspace ? localValue<string>(`last:${machine}:${workspace.id}`) : null;
  const continued = owned.find(item => item.id === last) ?? [...owned].sort((a,b) => (b.messagePreview?.atMs ?? b.updatedAtMs)-(a.messagePreview?.atMs ?? a.updatedAtMs))[0];
  const drafts = draftIdentities(machine).filter(item => item.workspaceId === workspace?.id).filter(item => {const content = readLocalDraft(`${machine}:${item.localId}`); return content.text || content.attachments.length || content.missingAttachments;});
  const browse = (id: string | null) => {
    window.history.replaceState({...window.history.state, genehubSpace: {id: selectedId, surface}}, "");
    window.history.pushState({...window.history.state, genehubSpace: {id, surface: "sessions"}, genehubParent: true}, "");
    setSelectedId(id); setSurfaceState("sessions"); setQuery("");
  };
  const visible = query.trim()
    ? [...nodes.values()].filter((item) => `${item.workspace.name} ${item.breadcrumb}`.toLowerCase().includes(query.trim().toLowerCase()))
    : workspace ? node?.children ?? [] : [...nodes.values()].sort((a, b) => a.workspace.name.localeCompare(b.workspace.name));
  return <section className="flex h-full min-h-0 flex-col" aria-label="空间浏览">
    <header className="shrink-0 border-b border-line px-4 py-3 md:px-6">
      <div className="flex items-center gap-2">{workspace && <button type="button" aria-label="返回空间" className="min-h-11 min-w-11 text-xl" onClick={() => { if (window.history.state?.genehubParent) window.history.back(); else if (surface !== "sessions") setSurface("sessions"); else browse(null); }}>‹</button>}<p className="text-xs text-muted">{deviceName}</p></div>
      <nav aria-label="空间路径" className="mt-2 flex flex-wrap items-center gap-1 text-sm">
        ...
        {workspace && <span className="truncate px-2">/ {workspace.name}</span>}
      </nav>
      {workspace ? <div className="mt-2 flex flex-wrap gap-1" aria-label="空间工具">
        {[["sessions", "空间资料"], ["files", "文件"], ["changes", "变更"], ["terminal", "终端"]].map(([id, label]) => <button key={id} type="button" aria-pressed={surface === id} className={`min-h-10 rounded-lg px-3 text-sm ${surface === id ? "bg-raised text-accent" : "text-muted hover:bg-raised"}`} onClick={() => { setSurface(id!); if (id === "terminal") setTerminals((current) => current.includes(workspace.id) ? current : [...current.slice(-3), workspace.id]); }}>{label}</button>)}
    ...
    {surface === "sessions" ? <div className="min-h-0 flex-1 overflow-y-auto p-4 md:px-6">
      <div className="mx-auto max-w-4xl">
        ...
        {workspace && !query && <div className="mb-6 space-y-3"><button type="button" className="min-h-12 w-full rounded-xl bg-accent px-4 py-3 text-left text-on-accent" onClick={() => continued ? onSession(continued.id) : onNewSession(workspace.id)}>{continued ? `${continued.managed?.userInteraction === "readOnly" ? "查看会话" : "继续会话"} · ${continued.title ?? "未命名会话"}` : "开始会话"}</button>{drafts.map(item => <button key={item.localId} type="button" className="block min-h-11 w-full truncate rounded-lg bg-raised px-4 text-left text-sm" onClick={() => onNewSession(workspace.id,item.localId)}>继续草稿 · {readLocalDraft(`${machine}:${item.localId}`).text || "附件草稿"}</button>)}</div>}
        <h2 className="mb-2 text-sm font-medium">{query ? "搜索结果" : workspace ? `子空间 · ${visible.length}` : "空间联系人"}</h2>
        <ul className="grid gap-2 sm:grid-cols-2" aria-label="空间联系人">
