...
export function WorkspaceBrowser({host, endpoint, deviceName, onNewSession, onDepthChange}: {
  ...
  return <section className="flex h-full min-h-0 flex-col" aria-label="专家目录">
    <header className="shrink-0 border-b border-line p-4"><div className="mx-auto max-w-3xl"><AgentDirectoryToolbar host={host} endpoint={endpoint} query={query} onQuery={setQuery} groups={groups} groupId={group?.id ?? ""} onGroup={setGroupId} onManage={() => setGroupsOpen(true)} /></div></header>
...
function AgentDirectoryToolbar({ host, endpoint, query, onQuery, groups, groupId, onGroup, onManage }: {
  ...
  return <div aria-label="专家目录工具栏" className="space-y-2">
    ...
    <label className="flex min-w-0 items-center gap-2 rounded-lg bg-raised px-3 text-muted"><Search size={16} className="shrink-0" /><input type="search" aria-label="搜索专家" placeholder="搜索专家" value={query} onChange={(e) => onQuery(e.target.value)} className="min-h-11 w-full min-w-0 bg-transparent text-sm outline-none" /></label>
  </div>;
}
