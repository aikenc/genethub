...
export function WorkspaceBrowser({host, endpoint, deviceName, onNewSession, selectedId}: {
  ...
  return <section className="flex h-full min-h-0 flex-col" aria-label="项目列表">
    ...
    <ListScroll label="项目列表滚动区域"><AgentList selectedId={selectedId} workspaces={workspaces} sessions={sessions} onPick={onNewSession} query={query} memberIds={group?.workspaceIds} deviceName={deviceName} /></ListScroll>
...
function AgentDirectoryToolbar({ host, endpoint, query, onQuery, groups, groupId, onGroup, onManage, searchOpen, onSearch }: {
  ...
  return <div aria-label="项目列表工具栏" className="space-y-2">
    <ListToolbar label="项目导航工具栏" searchLabel="搜索项目" searchOpen={searchOpen} onSearch={onSearch}>
      <ListGroupSelect label="项目分组筛选" allLabel="全部项目" value={groupId} groups={groups} onChange={onGroup}/>
      <OpenProject host={host} endpoint={endpoint} />
    </ListToolbar>
    <div className="flex min-h-9 items-center justify-end"><button type="button" aria-label="管理项目分组" className="min-h-9 rounded-lg px-2 text-xs text-muted hover:bg-raised" onClick={onManage}>管理分组</button></div>
    {searchOpen && <ListSearch label="搜索项目" value={query} onChange={onQuery} onClose={onSearch}/>}
