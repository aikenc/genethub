...
export function AgentDetails({ workspace, deviceName, compact = false }: { workspace: WorkspaceInfo; deviceName: string; compact?: boolean }) {
  ...
  return <section aria-label={`${workspace.name} 的资料`} className="min-w-0 space-y-4">
    ...
    <details open={!compact}><summary className="mb-2 cursor-pointer text-sm font-medium">Root 目录 · {folders.length}</summary><ul className="space-y-2">{folders.map((folder) => <li key={folder.root} className="rounded-lg border border-line bg-raised/40 p-3"><p className="text-sm font-medium">{folder.name}</p><p className="mt-1 break-all text-xs leading-5 text-muted">{folder.root}</p></li>)}</ul></details>
...
export function AgentDetailsDialog({ workspace, deviceName, onClose, onBrowse }: { workspace: WorkspaceInfo; deviceName: string; onClose(): void; onBrowse?(): void }) {
  return <WorkspaceDetailsDialog title="Agent详情" onClose={onClose}>
