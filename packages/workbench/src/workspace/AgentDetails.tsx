...
export function AgentDetails({ workspace, deviceName, compact = false }: { workspace: WorkspaceInfo; deviceName: string; compact?: boolean }) {
  ...
  return <section aria-label={`${workspace.name} 的资料`} className="min-w-0 space-y-4">
    {!compact && <div className="flex items-center gap-3"><AgentAvatar id={workspace.id} name={workspace.name} /><div className="min-w-0"><h2 className="break-words text-lg font-semibold">{workspace.name}</h2><p className="text-xs text-muted">{components.join(" / ") || (workspace.workspaceFile ? "多目录项目" : "单目录项目")}</p></div></div>}
    <dl className="space-y-3 text-sm">
      ...
      {parent && <Fact label="上级项目">{parent.name}</Fact>}
      <Fact label="会话数量">
...
export function AgentDetailsDialog({ workspace, deviceName, onClose, onBrowse }: { workspace: WorkspaceInfo; deviceName: string; onClose(): void; onBrowse?(): void }) {
  return <WorkspaceDetailsDialog title="资料与配置" fullScreenOnMobile onClose={onClose}>
    ...
    {onBrowse && <button type="button" className="mt-4 min-h-11 w-full rounded-lg bg-accent text-white" onClick={onBrowse}>打开项目面板</button>}
