...
describe("connecting a machine to a Hub", () => {
    it("explains that a Hub is optional before asking for one", () => {
        render(_jsx(Pairing, { status: { state: "unpaired" }, host: host(), onPair: async () => { }, onUnpair: async () => { } }));
    ...
    it("will not send an empty address", async () => {
        ...
        render(_jsx(Pairing, { status: { state: "unpaired" }, host: host(), onPair: onPair, onUnpair: async () => { } }));
    ...
    it("puts the code on screen and sends the user to the real browser to approve it", async () => {
        ...
        render(_jsx(Pairing, { status: {
            ...
            }, host: host({ openExternal }), onPair: async () => { }, onUnpair: async () => { } }));
    ...
    it("tells the difference between not paired and paired but unreachable", () => {
        const { rerender } = render(_jsx(Pairing, { status: {
                state: "paired",
                hubUrl: "https://hub.example.com",
                machineId: "m_1",
                online: true,
            }, host: host(), onPair: async () => { }, onUnpair: async () => { } }));
        expect(screen.getByText(/远程可达/)).toBeInTheDocument();
        rerender(_jsx(Pairing, { status: {
                state: "paired",
                hubUrl: "https://hub.example.com",
                machineId: "m_1",
                online: false,
    ...
    it("keeps a failure on screen with the reason, rather than reverting silently", () => {
        render(_jsx(Pairing, { status: { state: "failed", hubUrl: "https://hub.example.com", message: "配对码已过期" }, host: host(), onPair: async () => { }, onUnpair: async () => { } }));
    ...
    it("lets the owner disconnect", async () => {
        ...
        render(_jsx(Pairing, { status: {
                ...
                machineId: "m_1",
                online: true,
            }, host: host(), onPair: async () => { }, onUnpair: onUnpair }));
