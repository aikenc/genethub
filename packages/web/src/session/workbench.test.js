...
describe("what the user sees in a session", () => {
    it("shows the reply as it streams rather than only when it finishes", () => {
        ...
        render(_jsx(Timeline, { state: state }));
    ...
    it("renders a shell call as its command and output", () => {
        render(_jsx(ToolCallView, { name: "bash", status: "ok", detail: { kind: "shell", command: "ls -a", output: "a\nb", exitCode: 0 } }));
    ...
    it("colours an edit so the change is readable at a glance", () => {
        render(_jsx(ToolCallView, { name: "edit", status: "ok", detail: { kind: "edit", path: "src/main.rs", diff: "@@ -1 +1 @@\n-old\n+new" } }));
    ...
    it("shows a tool it has never heard of instead of dropping it", async () => {
        render(_jsx(ToolCallView, { name: "teleport", status: "error", detail: { kind: "unknown", raw: { arguments: { destination: "mars" } } } }));
...
describe("the controls offered to the user", () => {
    it("does not offer a model picker for an agent that cannot switch models", () => {
        ...
        render(_jsx(AgentControls, { agents: [fixed], agentId: "fixed", modelId: null, modeId: null, onPickAgent: () => { }, onPickModel: () => { }, onPickMode: () => { } }));
    ...
    it("leaves an agent that is not installed out of the picker entirely", () => {
        render(_jsx(AgentControls, { agents: [agent(), agent({ id: "opencode", label: "OpenCode", probe: { state: "notInstalled" } })], agentId: "genet", modelId: null, modeId: null, onPickAgent: () => { }, onPickModel: () => { }, onPickMode: () => { } }));
    ...
    it("sends on enter and keeps shift+enter for a new line", async () => {
        const onSend = vi.fn();
        render(_jsx(Composer, { running: false, onSend: onSend, onInterrupt: () => { } }));
    ...
    it("turns send into stop while a turn is running", async () => {
        ...
        render(_jsx(Composer, { running: true, onSend: () => { }, onInterrupt: onInterrupt }));
    ...
    it("asks for approval in the timeline and reports which option was chosen", async () => {
        ...
        render(_jsx(PermissionCard, { request: {
                id: "p1",
        ...
        for (const event of [
            {
                type: "item",
                turnId: "t1",
                item: { type: "userMessage", id: "u1", text: "写个文件", attachments: [] },
            },
            { type: "turnStarted", turnId: "t1" },
            { type: "item", turnId: "t1", item: call },
            {
                type: "itemDelta",
                turnId: "t1",
                itemId: "c1",
                delta: { kind: "toolStatus", status: "ok" },
            },
            {
                type: "item",
                turnId: "t1",
                item: { type: "assistantMessage", id: "a1", text: "写好了。" },
            },
            {
                type: "turnCompleted",
                turnId: "t1",
