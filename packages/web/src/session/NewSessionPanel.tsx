...
/**
 * How many projects the panel offers before the rest are folded away.
 *
 ...
 * always among these.
 */
export const NEW_SESSION_PROJECT_PREVIEW_LIMIT = 4;

/**
 * What an unstarted conversation shows instead of an empty transcript.
 *
 * Which project a new chat belongs to used to be readable only as a highlight
...
export function NewSessionPanel() {
  ...
  // on every render meant a project jumped to the top of the grid under the
  ...
  const visible = showAllProjects
    ? ordered
    : ordered.slice(0, NEW_SESSION_PROJECT_PREVIEW_LIMIT);

  return (
    <div className="mx-auto h-full min-w-0 max-w-chat overflow-y-auto px-3 py-4">
      <h2 className="text-sm font-medium text-fg">新会话</h2>
      <p className="mt-0.5 text-xs text-muted">选好项目，然后在下面直接说要做什么。</p>

      <section className="mt-3 min-w-0" aria-labelledby="new-session-project">
        <h3
        ...
        {ordered.length > visible.length || showAllProjects ? (
          <button
            type="button"
          ...
          >
            {showAllProjects ? "收起" : `更多 ${ordered.length - visible.length}`}
          </button>
        ) : null}
      </section>
...
/**
 * The project the panel opened on, then the ones worked in most recently.
 *
 * The sidebar's selection stays first no matter how long ago it was touched —
 * being offered four projects that do not include the one you are looking at
