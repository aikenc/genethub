//! What the operating system will hold a process to, when one is started for
//! somebody who is not sitting at this machine.
//!
//! The layer above this one decides *who* may ask (`apps/daemon/src/authz.rs`).
//! This one is the only layer that can answer *what the process can actually
//! touch*, and it is the only one that is enforced by something other than our
//! own code: a shell we start can run `python -c` and walk straight past any
//! list of allowed command names we might have kept
//! (`genet-remote-execution.md` §7.1).
//!
...
//! fork and exec, so a confined process is started by re-running a native
//! front door as [`CONFINE_ARG`]: it restricts itself, then execs what was
//! asked for. If it cannot restrict itself, it exits instead of exec'ing — the
//! failure mode is a terminal that will not open, not one that opens with
//! nothing holding it back. That wrapper must reach us before any thread
//! starts: a multi-threaded process cannot create a user namespace at all.
//!
//! That last rule is also why a wasm guest can still have this. The policy is
//! decided in the guest and the wrapper argv is built there ([`Policy::wrap`]),
//! because none of that needs a kernel; the probe and the restriction itself
//! happen out here, in a process that has one.
...
/// the guest names can play the part.
...
impl Policy {
    ...
    ///
    /// The wrapper has to be a native binary that answers to [`CONFINE_ARG`],
    /// and the caller may be the wasm guest, which is neither native nor a
    /// binary. So the search runs from most specific to least: what a test was
    /// told to use, then the front-door CLI the shell named for the guest,
    /// then this process itself — the answer a native daemon has always given,
    /// and the wrong one only where `current_exe` is a component.
    pub fn wrap(&self, program: &Path) -> Result<Vec<PathBuf>> {
        let helper = match std::env::var_os(CONFINE_COMMAND_ENV)
            .or_else(|| std::env::var_os("GENEHUB_CLI"))
...
#[cfg(target_os = "linux")]
fn landlock_abi() -> std::result::Result<i64, String> {
