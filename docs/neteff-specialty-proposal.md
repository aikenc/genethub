...
产品侧的 Preview 信息面板另行暴露单次入口文件的五项可观察数据：文件大小、总等待、首字节、正文
平均速度和 DATA 分片数/最大分片，并标出 WebSocket、Fabric Relay 或 WebRTC 路径。这些数据用于定位
真实用户的一次慢预览，但不冒充专项指标：首字节同时包含请求 RTT、daemon 扫描和排队，浏览器也
没有三条路径通用的独立 RTT API。严格 RTT 与 TCP 利用率仍只由本专项在同一受控链路上测量。
