...
### D-B：~~沙箱强度由 Workflow 声明~~ → **平台不做沙箱**（2026-09-21 推翻重写）

初稿写"平台强制严格"，二稿改成"由包声明、挑战卡逐条批准"。**两稿都错，本节整体作废并重写。**

二稿看似符合 J1（把策略交还给项目），实际仍然假设"该有一道沙箱，只是谁来定的问题"。
这个前提本身不成立，三条理由任一独立足够：

1. **它挡不住任何人。** `pack.script` 的调用者是 `SessionController`，权限只有
   `Handshake|Read|Session`，永远拿不到 `pty:unconfined`，所以**恒被沙箱**；而同一个 Agent
   自己跑 bash（`adapter/*.rs` 直接 spawn）**完全不受限**。同一件事换条路就绕过——
   沙箱只拦住了老实走声明式能力的那条路，是在惩罚正确用法。
2. **它把能力废掉了。** 沙箱可写只有 workspace roots，只读只有 `/usr /bin /sbin /lib /etc /opt`，
   **没有 `$HOME`、没有 `/tmp`、没有 `/proc`**。凡是要用 `~/.ssh`、`~/.npmrc`、`~/.aws`、
   `~/.itch/credentials` 的脚本一律不能工作——而"提交到远端、发布到外部平台"正是本能力的立身场景。
3. **这一层不归平台管。** 隔离是**部署决策**：用户认为这台机器不该被完全触达，就把 GeneHub
   装进虚拟机或容器。平台替他在代码里做这个决定，正是 J1 禁止的"平台定义它不必亲自执行的结构"。

**最终分层**：

- **平台保留的只有机制，不是权限**：输出读取上限与超时后回收进程组（护的是 **daemon 自己**
  不被挂死脚本拖垮，不是限制脚本能做什么）；argv 传参不经 shell（护的是**调用方**不被
  节点 `input` 注入出第二条命令，脚本自己想开 shell 随时可以）。
- **其余全部放开**：包自己声明 `interpreter`（含 `bash`）、`args`、`env`、`cwd`；`script`
  可以是包内相对路径，也可以是 `PATH` 上的任意程序（`blender`/`ffmpeg`/`docker`）；
  扩展名不再有白名单；不做路径包含判定；`timeoutSeconds` 无上限（渲染和构建本来就跑几小时）。
- **信任锚不变**：`workflow build --apply` 的人类挑战卡 + 锚定 `source_digest`（J2）。
  批准的含义是"这份源码可以在我的机器上执行"，本来就是全权，不该再假装有第二道墙。
...
- **D1 `uses` 注册表**：新增唯一一个 `uses: pack.script`。平台**必须亲自执行**的那部分契约
  （不开放）：argv 传参不经 shell、超时后回收整个进程组、输出读取上限、
  stdout 必须有界结构化 JSON、`idempotencyKey` 透传。平台不理解语义。
  **不做沙箱**：解释器、参数、环境变量、工作目录、可执行程序全部由包自己声明，
  可执行 `PATH` 上任意程序，不做路径包含判定——见重写后的 D-B。
...
6. 不引入包可自声明的 trusted 标志位。**信任只有一个来源**：人类在 `workflow build --apply`
   的挑战卡上批准了这份源码，并锚定其 `source_digest`（J2）。批准即全权，平台不再额外设墙。
7. **平台不做沙箱**（2026-09-21 新增，见重写后的 D-B）。隔离是部署决策，属于"用户拿自己的
   机器做什么"，不属于平台。要隔离就把 GeneHub 装进虚拟机或容器。平台只保留**机制性**的
   边界——超时回收、输出上限、argv 不经 shell——它们保护的是 daemon 自身与调用方，
   不是对被调用方的权限限制。任何"为了安全"在这一层加限制的改动，都必须先推翻本条。
...
5. ~~沙箱强度由包声明、经挑战卡逐条批准~~ — **本条撤销**（2026-09-21）。它预设"该有一道沙箱"，
   而该前提已被推翻：平台不做沙箱，隔离交给部署（虚拟机/容器）。判据改为**反向**验收——
   包脚本可以自选解释器（含 shell）、自带环境变量与工作目录、执行 `PATH` 上任意程序、
   读写 workspace 之外的路径；见重写后的 D-B 与不变量 7；
...
**第 5 条撤销并反向落地（2026-09-21 追加提交）**：原先 `pack.script` 复用了 Session confinement
策略，收尾时把它作为「待补的缺口」记录在此。用户指出这个方向本身是错的——**不该在这一层做沙箱**，
核查后确认该限制既无效又有害（理由见重写后的 D-B）。因此不是去补声明式沙箱，而是**整体移除**：

- `mod.rs` 不再调用 `isolation::required_for`，`script::run` 不再接受 confinement 参数；
- 解释器白名单删除，改为包声明 `interpreter`（可为 `bash`，也可省略直接执行）；
- 新增 `args`/`env`/`cwd` 由包声明；`script` 可为包内路径或 `PATH` 上任意程序；
- `resolve_script` 从「拒绝越界」改为「解析优先包内、其余交给 OS」；
- `MAX_TIMEOUT_SECONDS` 上限删除，`timeoutSeconds` 由包决定。

保留的只有机制：输出读取上限、超时回收进程组、argv 不经 shell。新增两条反向单测——
`a_package_names_its_own_interpreter_including_a_shell` 与
`a_script_receives_its_declared_environment_and_is_not_confined`（后者实证脚本能读到
workspace 与包目录之外的文件），把「不受限」钉成可回归的事实而非注释里的承诺。
