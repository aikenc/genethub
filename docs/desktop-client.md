...
它不定时查，也不静默装。人点「下载」时，去拉安装包的是 **daemon** 而不是外壳（`update.download`，见 [daemon.md](./daemon.md) §7）：Linux 上没有这个外壳，而且这样一来手机上按下的下载也能在手机上看着它走完。文件落在 `<data>/updates/`，进度作为状态推给每个连着的客户端——所以关掉设置页、甚至关掉窗口，下载都还在走，重新打开时那个框还在。
