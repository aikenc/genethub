...
页面统一使用现有浏览器历史栈。侧栏选择会话或项目属于一级切换，宽屏保留列表时无需返回按钮；从正文打开执行记录、工作会话、子项目或资料/文件等深层页面时入栈，宽屏也显示返回。项目主页的页签是同级切换；菜单详情与目录中的文件入口是下一级。手机一级详情返回本列表，浏览器前进/后退同步恢复页面。

项目会话的筛选与滚动位置按历史条目保存；会话草稿沿用原有持久化。弹框保持独立开关，不写页面历史。Agent 自身发送/运行状态和小队任务状态分别呈现在输入框与会话头部，用户提问只进入当前 Agent。
