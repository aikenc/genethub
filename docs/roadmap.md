...
| CI 影响分类 | `scripts/ci-classify.mjs` 分三层：`app`（Win/mac 矩阵）只在 ABI 边界 `wit/` 或**带平台分支且编进原生二进制**的源码（host/cli/desktop/native/frontdoor/scripts/.github/Cargo.lock）变更时为真；`packages/proto`、`packages/identity`、`packages/http`、`apps/daemon` 没有平台分支，只开一个 ubuntu 编译兜底；guest/agent/workbench/relay/testing/docs 不进 GitHub 重腿。`false` 的 job 不分配 runner。漏标 fail closed 到全量。不输出 Live/App `release_type` | CLI/Host 解耦已落地：`apps/cli` 不再链 `genet-daemon`，`apps/host` 不再链 `genehub-proto`，指纹树同步收紧（`cli-thin-forwarder.md` §6）；另有发布分类器驱动 candidate/promote |
...
- Agent 的 subagents / MCP / 真压缩（见 [builtin-agent.md](./builtin-agent.md) §8）
