...
| 机器离线 | 前端显示离线并给出可操作提示 |
| carrier 升级 | 同机 loopback 与跨设备 Fabric baseline 各自可用；远端双方启用时尝试 WebRTC，失败后新请求继续走同一 v3 baseline，且不自动重放在途请求 |
...
| Windows | 只有跨平台 App 变更才进 GitHub CI。发布流水线（`.github/workflows/release.yml`）构建安装包；桌面 CI 在 Windows runner 上执行壳层 library、wiring、真实 daemon supervision 与 lint。此时 `genehub_guest.wasm` 只在 Linux `guest` job 编译，Win/mac 下载同一份。host/CLI 用源码指纹缓存，命中则跳过 rustc。wasm / Web 日常在 Linux 开发机构建。凡是会拉起 daemon 的 CI 腿都启动 iterate 剖面的 CLI/host，不要用 debug host（本机 506MB / 6s） |
| macOS | 与 Windows 同一套壳层测试与跳过规则；正式安装包等待签名、公证完成后再发布 |
