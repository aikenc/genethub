...
**去重降级为尽力而为。** 写入前的存在性判断只查进程内的 id→引用表，覆盖「同一次运行里反复读同一个文件」这个主要场景。跨 daemon 重启的重复内容会存两份——这是故意的：为了跨重启去重就得在打开会话时加载全量 id 索引，等于把刚砍掉的 O(N) 请回来。两份副本各自有完整引用，读取都正确，只是多占磁盘。
...
| 写一条 blob | O(桶字节) 扫描去重 | O(1) 内存查重 + 追加 |
